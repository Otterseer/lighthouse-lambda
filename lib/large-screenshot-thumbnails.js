...
const Speedline = require('../../lighthouse/lighthouse-core/computed/speedline.js');
...
class LargeScreenshotThumbnails extends Audit {
	/**
	 * @return {LH.Audit.Meta}
	 */
	static get meta() {
		return {
			id: 'large-screenshot-thumbnails',
			scoreDisplayMode: Audit.SCORING_MODES.INFORMATIVE,
			title: 'Large Screenshot Thumbnails',
	...
	static async _audit(artifacts, context) {
		...
		for (let i = 1; i <= NUMBER_OF_THUMBNAILS; i++) {
			...
			if (cachedThumbnail) {
				...
				const thumbnailImageData = LargeScreenshotThumbnails.scaleImageToThumbnail(imageData);
				base64Data = jpeg.encode(thumbnailImageData, 100).data.toString('base64');
...
module.exports = LargeScreenshotThumbnails;
